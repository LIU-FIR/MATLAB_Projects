function [circ_buffer] = oversampled_pfb_fir2(x,h,M,R)
...
xd = x(1:end-M/2); % 信号延迟M/2点
xtr = x(M/2+1:end); % 在延迟信号有效值出现处截取原信号
Ri = 2*R; % 每一分支上内插后滤波器的长度
W = floor((length(x)-M/2)/(M/2*Ri)); % 按M/2通道排列和分支多相滤波器长度为R组织，计算信号“块”数。
xtr_p = reshape(xtr(1:M/2*Ri*W),[M/2 W*Ri]);
xd_p = reshape(xd(1:M/2*Ri*W),[M/2 W*Ri]);
xp = cat(1,xtr_p,xd_p); % 拼接为M通道的数据
% xp = cat(1,xd_p,xtr_p); % 拼接为M通道的数据
